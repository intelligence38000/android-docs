...
**Device location**: In the adjacent diagram, the blue dot with white stroke represents the device location.

**Bearing**: The bearing tells you the direction a user's device is facing. It is an angle clockwise from true north between 0 and 359. For example, if the device is facing north, the bearing will be 0°, and if the user is facing due east, the bearing will be 90°. In the adjacent diagram, the pink arrow represents the direction that the device is facing (which is due west or 270°).
